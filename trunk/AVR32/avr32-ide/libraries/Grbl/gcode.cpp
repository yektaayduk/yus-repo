...
void Grbl::GCode::init() 
{
  memset(&gc_state, 0, sizeof(gc_state));
  
  // Load default G54 coordinate system.
  if (!(parent->m_settings.read_coord_data(gc_state.modal.coord_select,gc_state.coord_system))) { 
...
void Grbl::GCode::sync_position() 
{
  uint8_t i;
  for (i=0; i<N_AXIS; i++) {
    gc_state.position[i] = parent->m_system.sys.position[i]/parent->m_settings.settings.steps_per_mm[i];
  }
}


uint8_t Grbl::GCode::check_same_position(float *pos_a, float *pos_b) 
{
  uint8_t idx;
...
uint8_t Grbl::GCode::execute_line(char *line) 
{
  /* -------------------------------------------------------------------------------------
  ...
  */  
  ...
  if ( bit_istrue(command_words,bit(MODAL_GROUP_G12)) ) { // Check if called in block
    ...
    if (gc_state.modal.coord_select != gc_block.modal.coord_select) {
      if (!(parent->m_settings.read_coord_data(gc_block.modal.coord_select,coordinate_data))) { FAIL(STATUS_SETTING_READ_FAIL); } 
    }
  }
  ...
  switch (gc_block.non_modal_command) {
    case NON_MODAL_SET_COORDINATE_DATA:  
      ...
      if (!parent->m_settings.read_coord_data(coord_select,parameter_data)) { FAIL(STATUS_SETTING_READ_FAIL); } // [EEPROM read fail]
    ...
    default:
      ...
      switch (gc_block.non_modal_command) {        
        case NON_MODAL_GO_HOME_0: 
          ...
          if (!parent->m_settings.read_coord_data(SETTING_INDEX_G28,parameter_data)) { FAIL(STATUS_SETTING_READ_FAIL); }
          break;
        case NON_MODAL_GO_HOME_1:
          ...
          if (!axis_words) { axis_command = AXIS_COMMAND_NONE; } // Set to none if no intermediate motion.
          if (!parent->m_settings.read_coord_data(SETTING_INDEX_G30,parameter_data)) { FAIL(STATUS_SETTING_READ_FAIL); }
          break;
      ...
      switch (gc_block.modal.motion) {
        ...
        case MOTION_MODE_CW_ARC: case MOTION_MODE_CCW_ARC:
          ...
          if (value_words & bit(WORD_R)) { // Arc Radius Mode  
            ...
            if (check_same_position(gc_state.position, gc_block.values.xyz)) { FAIL(STATUS_GCODE_INVALID_TARGET); } // [Invalid target]
  ...
  */ 
  ...
  if (gc_state.spindle_speed != gc_block.values.s) { 
    ...
    if (gc_state.modal.spindle != SPINDLE_DISABLE) { parent->m_spindle.run(gc_state.modal.spindle, gc_state.spindle_speed); }
  }
  ...
  if (gc_state.modal.spindle != gc_block.modal.spindle) {
    ...
    parent->m_spindle.run(gc_state.modal.spindle, gc_state.spindle_speed);
  }

  // [8. Coolant control ]:  
  if (gc_state.modal.coolant != gc_block.modal.coolant) {
    gc_state.modal.coolant = gc_block.modal.coolant;
    parent->m_coolant.run(gc_state.modal.coolant);
  }
  
  // [9. Enable/disable feed rate or spindle overrides ]: NOT SUPPORTED

  // [10. Dwell ]:
  if (gc_block.non_modal_command == NON_MODAL_DWELL) { parent->m_mc.dwell(gc_block.values.p); }
  ...
  switch(gc_block.non_modal_command) {
    case NON_MODAL_SET_COORDINATE_DATA:    
      parent->m_settings.write_coord_data(coord_select,parameter_data);
    ...
    case NON_MODAL_GO_HOME_0: case NON_MODAL_GO_HOME_1: 
      ...
      if (axis_command) {
          ...
          parent->m_mc.line(gc_block.values.xyz, -1.0, false, gc_block.values.n);
        #else
          parent->m_mc.line(gc_block.values.xyz, -1.0, false);
        #endif
      }
      #ifdef USE_LINE_NUMBERS
        parent->m_mc.line(parameter_data, -1.0, false, gc_block.values.n); 
      #else
        parent->m_mc.line(parameter_data, -1.0, false); 
      #endif
      memcpy(gc_state.position, parameter_data, sizeof(parameter_data));
      break;
    case NON_MODAL_SET_HOME_0: 
      parent->m_settings.write_coord_data(SETTING_INDEX_G28,gc_state.position);
      break;
    case NON_MODAL_SET_HOME_1:
      parent->m_settings.write_coord_data(SETTING_INDEX_G30,gc_state.position);
      break;
    case NON_MODAL_SET_COORDINATE_OFFSET:
  ...
  if (gc_state.modal.motion != MOTION_MODE_NONE) {
    if (axis_command == AXIS_COMMAND_MOTION_MODE) {
      switch (gc_state.modal.motion) {
        ...
        case MOTION_MODE_LINEAR:
          #ifdef USE_LINE_NUMBERS
            parent->m_mc.line(gc_block.values.xyz, gc_state.feed_rate, gc_state.modal.feed_rate, gc_block.values.n);
          #else
            parent->m_mc.line(gc_block.values.xyz, gc_state.feed_rate, gc_state.modal.feed_rate);
          #endif
          break;
        case MOTION_MODE_CW_ARC: case MOTION_MODE_CCW_ARC:
          #ifdef USE_LINE_NUMBERS
            parent->m_mc.arc(gc_state.position, gc_block.values.xyz, gc_block.values.ijk, gc_block.values.r, 
        ...
        case MOTION_MODE_PROBE:
          ...
          #ifdef USE_LINE_NUMBERS
            parent->m_mc.probe_cycle(gc_block.values.xyz, gc_state.feed_rate, gc_state.modal.feed_rate, gc_block.values.n);
          #else
            parent->m_mc.probe_cycle(gc_block.values.xyz, gc_state.feed_rate, gc_state.modal.feed_rate);
          #endif
      }
  ...
  if (gc_state.modal.program_flow) { 
    parent->m_protocol.buffer_synchronize(); // Finish all remaining buffered motions. Program paused when complete.
    parent->m_system.sys.auto_start = false; // Disable auto cycle start. Forces pause until cycle start issued.
  
    // If complete, reset to reload defaults (G92.2,G54,G17,G90,G94,M48,G40,M5,M9). Otherwise,
    // re-enable program flow after pause complete, where cycle start will resume the program.
    if (gc_state.modal.program_flow == PROGRAM_FLOW_COMPLETED) { parent->m_mc.reset(); }
