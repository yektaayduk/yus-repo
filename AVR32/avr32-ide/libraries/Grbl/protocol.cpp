...
void Grbl::Protocol::execute_runtime()
{
  uint8_t rt_exec = parent->m_system.sys.execute; // Copy to avoid calling volatile multiple times
  if (rt_exec) { // Enter only if any bit flag is true
    ...
    if (rt_exec & (EXEC_ALARM | EXEC_CRIT_EVENT)) {      
      parent->m_system.sys.state = STATE_ALARM; // Set system alarm state
      ...
      if (rt_exec & EXEC_CRIT_EVENT) {
        if (rt_exec & EXEC_ALARM) { parent->m_report.alarm_message(ALARM_LIMIT_ERROR); }
        else { parent->m_report.alarm_message(ALARM_PROBE_FAIL); }
        parent->m_report.feedback_message(MESSAGE_CRITICAL_EVENT);
        bit_false_atomic(parent->m_system.sys.execute,EXEC_RESET); // Disable any existing reset
        do { 
        ...
        } while (bit_isfalse(parent->m_system.sys.execute,EXEC_RESET));
      ...
      // Standard alarm event. Only abort during motion qualifies.
        ...
        parent->m_report.alarm_message(ALARM_ABORT_CYCLE);
      }
      bit_false_atomic(parent->m_system.sys.execute,(EXEC_ALARM | EXEC_CRIT_EVENT));
    } 
  
    // Execute system abort. 
    if (rt_exec & EXEC_RESET) {
      parent->m_system.sys.abort = true;  // Only place this is set true.
      return; // Nothing else to do but exit.
    }
    
    // Execute and serial print status
    if (rt_exec & EXEC_STATUS_REPORT) { 
      parent->m_report.realtime_status();
      bit_false_atomic(parent->m_system.sys.execute,EXEC_STATUS_REPORT);
    }
    ...
    if (rt_exec & EXEC_FEED_HOLD) {
      ...
      if (parent->m_system.sys.state == STATE_CYCLE) {
        parent->m_system.sys.state = STATE_HOLD;
        parent->m_stepper.update_plan_block_parameters();
        parent->m_stepper.prep_buffer();
        parent->m_system.sys.auto_start = false; // Disable planner auto start upon feed hold.
      }
      bit_false_atomic(parent->m_system.sys.execute,EXEC_FEED_HOLD);
    }
        
    // Execute a cycle start by starting the stepper interrupt begin executing the blocks in queue.
    if (rt_exec & EXEC_CYCLE_START) { 
      if (parent->m_system.sys.state == STATE_QUEUED) {
        parent->m_system.sys.state = STATE_CYCLE;
        parent->m_stepper.prep_buffer(); // Initialize step segment buffer before beginning cycle.
        parent->m_stepper.wake_up();
        if (bit_istrue(parent->m_settings.settings.flags,BITFLAG_AUTO_START)) {
          parent->m_system.sys.auto_start = true; // Re-enable auto start after feed hold.
        } else {
          parent->m_system.sys.auto_start = false; // Reset auto start per settings.
        }
      }    
      bit_false_atomic(parent->m_system.sys.execute,EXEC_CYCLE_START);
    }
    ...
    if (rt_exec & EXEC_CYCLE_STOP) {
      if ( parent->m_plan.get_current_block() ) { parent->m_system.sys.state = STATE_QUEUED; }
      else { parent->m_system.sys.state = STATE_IDLE; }
      bit_false_atomic(parent->m_system.sys.execute,EXEC_CYCLE_STOP);
    }
  ...
  if (parent->m_system.sys.state & (STATE_CYCLE | STATE_HOLD | STATE_HOMING)) { parent->m_stepper.prep_buffer(); }  
